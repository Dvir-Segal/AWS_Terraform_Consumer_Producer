...
resource "aws_iam_role_policy" "prometheus_policy" {
  ...
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [
      { Effect = "Allow", Action = ["ecs:ListTasks", "ecs:DescribeTasks", "ec2:DescribeInstances", "ec2:DescribeTags", "ec2:DescribeNetworkInterfaces", "cloudmap:ListServices", "cloudmap:ListNamespaces", "cloudmap:ListInstances"], Resource = "*" },
      { Effect = "Allow", Action = ["elasticfilesystem:ClientMount", "elasticfilesystem:ClientRootAccess", "elasticfilesystem:ClientWrite", "elasticfilesystem:DescribeMountTargets", "elasticfilesystem:DescribeFileSystems"], Resource = aws_efs_file_system.monitoring_efs.arn }
    ]
  })
}
...
resource "aws_efs_access_point" "prometheus_ap" {
  file_system_id = aws_efs_file_system.monitoring_efs.id
  posix_user { gid = 65534, uid = 65534 }
  root_directory { path = "/prometheus", creation_info { owner_gid = 65534, owner_uid = 65534, permissions = "0755" } }
}
...
resource "aws_ecs_task_definition" "grafana_task" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "GF_AUTH_ANONYMOUS_ENABLED", value = "true" }, # NOT for production
...
resource "aws_efs_access_point" "grafana_ap" {
  file_system_id = aws_efs_file_system.monitoring_efs.id
  posix_user { gid = 65534, uid = 65534 }
  root_directory { path = "/grafana", creation_info { owner_gid = 65534, owner_uid = 65534, permissions = "0755" } }
}
...
resource "aws_lb_target_group" "grafana_tg" {
  ...
  target_type = "ip"
  vpc_id      = aws_vpc.main.id
  health_check { path = "/api/health", interval = 30, timeout = 5, healthy_threshold = 2, unhealthy_threshold = 2, matcher = "200" }
}
...
resource "aws_lb_listener" "grafana_listener" {
  ...
  protocol          = "HTTP"
  default_action { type = "forward", target_group_arn = aws_lb_target_group.grafana_tg.arn }
}
...
resource "aws_ecs_service" "grafana_service" {
  ...
  load_balancer { target_group_arn = aws_lb_target_group.grafana_tg.arn, container_name = "grafana", container_port = 3000 }
  depends_on = [aws_efs_mount_target.az1, aws_efs_mount_target.az2, aws_efs_access_point.grafana_ap, aws_lb_listener.grafana_listener]
}


# --- UPDATES TO EXISTING RESOURCES ---

# Update Producer ECS Service to include Cloud Map registration
resource "aws_ecs_service" "microservice1" {
...
# Update existing ECS Task IAM Policy for Cloud Map and EFS permissions
resource "aws_iam_role_policy" "ecs_policy" {
  name = "ecs-basic-policy"
  role = aws_iam_role.ecs_task_role.id
  policy = jsonencode({
    ...
    Statement = [
      ...
      { Effect = "Allow", Action = ["ssm:GetParameter"], Resource = aws_ssm_parameter.auth_token.arn },
      # NEW: Allow ECS tasks to register with Cloud Map
      { Effect = "Allow", Action = ["servicediscovery:RegisterInstance", "servicediscovery:DeregisterInstance", "servicediscovery:Get*", "servicediscovery:List*"], Resource = "*" },
      # NEW: Allow ECS tasks to interact with EFS
      { Effect = "Allow", Action = ["elasticfilesystem:ClientMount", "elasticfilesystem:ClientRootAccess", "elasticfilesystem:ClientWrite", "elasticfilesystem:DescribeMountTargets", "elasticfilesystem:DescribeFileSystems"], Resource = aws_efs_file_system.monitoring_efs.arn }
    ]
  })
}
...
output "grafana_url" {
  ...
  value       = "http://${aws_lb.alb.dns_name}:3001"
}
